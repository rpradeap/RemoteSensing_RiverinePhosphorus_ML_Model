...
grid_final <- expand.grid( mtry = 2,  coefReg = 0.6,  coefImp = 0.5)
...
# R2.1 <- sampling_train_data(train.1, features_1, features_2, grid_final, trainmethod)
# R2.2 <- sampling_train_data(train.2, features_1, features_2, grid_final, trainmethod)
# R2.3 <- sampling_train_data(train.3, features_1, features_2, grid_final, trainmethod)
# R2.4 <- sampling_train_data(train.4, features_1, features_2, grid_final, trainmethod)
# R2.5 <- sampling_train_data(train.5, features_1, features_2, grid_final, trainmethod)


#Select the parametric with the best Rsquare value based on above iterative results


train.1 <- Smote_train(train.1, features_2, 0.1, 27) 
train.4 <- Smote_train(train.4, features_2, 0.05, 14) 
train.2 <- undersample_filter(train.2, 0.05, 15, 0.275, 0.725) 
train.3 <- undersample_filter(train.3, 0.3, 15, 0.175, 0.825) 
train.5 <- undersample_filter(train.5, 0.05, 20, 0.35, 0.65)
